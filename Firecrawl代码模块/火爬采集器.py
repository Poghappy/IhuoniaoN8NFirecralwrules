...
class FirecrawlCollector:
    ...
    async def scrape_single_page(self, url: str, **kwargs) -> Optional[ArticleData]:
        ...
        async with self._semaphore:
            try:
                ...
                # 执行抓取
                def _scrape():
                    return self.firecrawl.scrape(url, **scrape_params)

                result = await self._retry_operation(_scrape)
                ...
                logger.error(f"抓取页面时发生错误 {url}: {e!s}")
                return None
    ...
    async def crawl_website(self, base_url: str, **kwargs) -> CrawlResult:
        ...
        try:
            ...
            # 执行爬取
            def _crawl():
                return self.firecrawl.crawl(base_url, **crawl_params)

            result = await self._retry_operation(_crawl)
            ...
            error_msg = f"爬取网站时发生错误 {base_url}: {e!s}"
            errors.append(error_msg)
            logger.error(error_msg)
    ...
    async def extract_structured_data(
        ...
        try:
            ...
            # 执行提取
            def _extract():
                return self.firecrawl.extract(**extract_params)

            result = await self._retry_operation(_extract)
            ...
            logger.error(f"结构化数据提取时发生错误 {url}: {e!s}")
            return None
    ...
    async def batch_scrape(self, urls: List[str], **kwargs) -> List[Optional[ArticleData]]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"批量抓取异常 {urls[i]}: {result!s}")
    ...
    def save_results(
        self, results: Union[CrawlResult, List[ArticleData]], output_path: str, format: str = "json"
    ) -> bool:
        """保存抓取结果
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    logger.error(f"操作失败，已达到最大重试次数 {max_retries}: {e!s}")
                    raise
...
async def example_usage():
    ...
    # 示例2: 爬取整个网站
    print("\n=== 爬取整个网站 ===")
    crawl_result = await collector.crawl_website("https://firecrawl.dev/blog", limit=5, max_depth=1)
    ...
    structured_data = await collector.extract_structured_data("https://firecrawl.dev/blog", schema)
    if structured_data:
        print(f"结构化数据: {structured_data}")

    # 示例4: 批量抓取
    print("\n=== 批量抓取 ===")
    urls = ["https://firecrawl.dev/blog", "https://docs.firecrawl.dev"]
