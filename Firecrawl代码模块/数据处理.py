...
class ProcessedArticle:
    ...
    # 扩展字段（用于API集成）
    source_url: Optional[str] = None  # 源URL（通常与url相同）
    external_id: Optional[str] = None  # 外部ID
    ...
    def to_huoniao_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "publish_time": self.publish_date.isoformat()
...
class ContentCleaner:
    ...
    def clean_html(self, html_content: str) -> str:
        ...
        try:
            ...
            self.logger.warning(f"HTML清洗失败: {e!s}")
            return html_content
    ...
    def extract_keywords(self, title: str, content: str, max_keywords: int = 10) -> List[str]:
        ...
        if jieba:
            try:
                ...
                self.logger.warning(f"jieba关键词提取失败: {e!s}")
...
class DataProcessor:
    ...
    def process_article(self, raw_data: Dict[str, Any]) -> Optional[ProcessedArticle]:
        ...
        try:
            ...
            # 设置扩展字段
            article.source_url = url  # source_url通常与url相同
            article.external_id = raw_data.get("external_id") or raw_data.get("id")
            article.images = raw_data.get("images", [])
            article.metadata = raw_data.get("metadata", {})
            ...
            self.logger.error(f"处理文章失败: {e!s}, URL: {raw_data.get('url', 'unknown')}")
            return None
