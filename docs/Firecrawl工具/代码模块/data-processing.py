...
try:
    from textstat import flesch_reading_ease  # noqa: F401
except ImportError:
    flesch_reading_ease = None  # noqa: F401
...
class ProcessedArticle:
    ...
    def to_huoniao_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "publish_time": (
                self.publish_date.isoformat() if self.publish_date else datetime.now().isoformat()
            ),
...
class ContentCleaner:
    ...
    def clean_html(self, html_content: str) -> str:
        ...
        Returns:
            str: 清洗后的HTML内容
        """
        if not html_content or not BS4_AVAILABLE or BeautifulSoup is None:
            return html_content
        ...
        try:
            ...
            self.logger.warning("HTML清洗失败: %s", str(e))
            return html_content
    ...
    def extract_keywords(self, title: str, content: str, max_keywords: int = 10) -> List[str]:
        ...
        if jieba:
            try:
                ...
                self.logger.warning("jieba关键词提取失败: %s", str(e))
...
class DataProcessor:
    ...
    def process_article(self, raw_data: Dict[str, Any]) -> Optional[ProcessedArticle]:
        ...
        try:
            ...
            if not title or not content or not url:
                self.logger.warning("文章数据不完整: %s", url)
                return None
            ...
            if not self.cleaner.filter_content(
                content_text, self.min_content_length, self.max_content_length
            ):
                self.logger.info("文章内容未通过过滤: %s", url)
                return None

            # 重复检测
            content_hash = self._calculate_content_hash(title + content_text)
            if self.enable_dedup and content_hash in self.content_hashes:
                self.logger.info("检测到重复内容: %s", url)
                return None
            ...
            if self.extract_summary:
                article.summary = self._generate_summary(content_text)
            ...
            self.logger.error("处理文章失败: %s, URL: %s", str(e), raw_data.get("url", "unknown"))
            return None
    ...
    def process_batch(self, raw_data_list: List[Dict[str, Any]]) -> List[ProcessedArticle]:
        ...
        self.logger.info("批量处理完成: %d/%d 篇文章", len(processed_articles), len(raw_data_list))
