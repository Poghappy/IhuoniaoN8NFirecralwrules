...
class BaseProcessor(BaseService):
    ...
    async def process(self, data: CollectionResult) -> BaseResponse:
        ...
        try:
            ...
            return BaseResponse(
                success=True, data=processed_data, message="Data processed successfully"
            )
    ...
    async def create_record(
        ...
        with self.get_session() as session:
            record = CollectionRecord(
                url=url, content=content, metadata=metadata, status="completed"
            )
            session.add(record)
            session.commit()
...
class ConfigManager:
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "database": {"url": "sqlite:///data/firecrawl.db"},
